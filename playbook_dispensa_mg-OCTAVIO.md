...
- `editais_rodeiro_domsilverio.md` — dossiês ativos (Rodeiro **NO-GO** / Dom Silvério NO-GO)
...
| P4 | Sul de MG / Zona da Mata | **RISCO ALTO** — Rodeiro (350 km, Zona da Mata) confirmou inviabilidade. So se 100% remoto E valor > R$ 15k/ano |

## 2.5. Filtro de viabilidade — deal-breakers (checar ANTES de investir tempo)

> **Lição Rodeiro/MG (09/04/2026):** edital de 54 páginas, escopo completo (PGR + PCMSO + LTCAT + LIP + 15 ASOs presenciais + 12x eSocial + 2 Palestras NR-1), equipe mínima 3 profissionais, subcontratação vedada, deslocamento ilimitado por conta da contratada. Valor: R$ 6.208/12 meses = prejuízo de R$ 4.000–5.700. Distância 350 km de Uberlândia. **Resultado: NO-GO.**
>
> **Insight central: DISTANCIA E O FATOR MATADOR.** Cameras fora do raio de ~200 km de Uberlandia sao inviaveis quando o edital exige deslocamento por conta da contratada + ASOs presenciais.

**Antes de analisar qualquer dispensa, passar por estes 6 filtros eliminatorios:**

| # | Deal-breaker | Regra | Por que mata |
|---|-------------|-------|-------------|
| 1 | **Distancia > 200 km de Uberlandia** | So vale se receita > R$ 15k/ano | Custo de viagem recorrente (combustivel + diaria + tempo Kleber) come a margem inteira |
| 2 | **ASOs presenciais obrigatorios + deslocamento por conta da contratada** | Calcular custo de N viagens/ano | Cada ida a 300 km custa ~R$ 600–800 (combustivel + pedagio + alimentacao). 4 viagens/ano = R$ 2.400–3.200 so em deslocamento |
| 3 | **Equipe minima 3+ profissionais (Eng + Medico + TST)** | Higilabor NAO tem TST proprio | Precisa contratar/terceirizar TST — custo extra + risco de nao atender clausula |
| 4 | **Subcontratacao vedada** | Nao pode usar parceiro local para ASOs/visitas | Elimina a opcao de ter medico parceiro na cidade destino — obriga deslocamento da equipe propria |
| 5 | **Medicoes quantitativas com certificado de calibracao** | Verificar se Higilabor tem equipamento calibrado | Ruido, calor, agentes quimicos com dosimetria exigem equipamento com certificado RBC vigente |
| 6 | **Valor < R$ 8.000/ano para escopo completo** | Escopo completo = PGR + PCMSO + LTCAT + ASOs + eSocial (+ extras como LIP, palestras) | Piso real de custo para escopo completo dentro de 150 km e R$ 10.000–12.000. Abaixo de R$ 8k = prejuizo certo |

**Decisao rapida:**
- 0 deal-breakers atingidos → **GO, montar proposta**
- 1 deal-breaker atingido → **avaliar se compensacao existe** (ex: distancia 250 km mas valor R$ 18k)
- 2+ deal-breakers → **NO-GO, nao investir mais tempo**
...
- Exemplos reais analisados: **Rodeiro/MG (R$ 6.208 — NO-GO: escopo completo 7 itens + 350 km + equipe 3 profissionais = prejuizo R$ 4–5,7k)** e **Dom Silvério/MG (R$ 2.006 — NO-GO por distância/margem)** — ver `editais_rodeiro_domsilverio.md`
...
Faixas sugeridas (**recalibradas 09/04/26 apos analise completa do edital Rodeiro — R$ 6.208 para escopo de 7 itens = prejuizo confirmado**):

### Faixa A — Cameras dentro de 150 km de Uberlandia
| Servidores | Preco anual | Lance agressivo (dispensa) |
|------------|-------------|----------------------------|
| ate 15 | R$ 4.800 | R$ 4.200 |
| 16–30 | R$ 6.500 | R$ 5.600 |
| 31–50 | R$ 8.400 | R$ 7.200 |
| 51–80 | R$ 10.500 | R$ 9.000 |

### Faixa B — Cameras entre 150–250 km (somar deslocamento)
| Servidores | Preco anual | Lance agressivo (dispensa) |
|------------|-------------|----------------------------|
| ate 15 | R$ 5.800 | R$ 5.200 |
| 16–30 | R$ 7.500 | R$ 6.800 |
| 31–50 | R$ 9.500 | R$ 8.500 |
| 51–80 | R$ 12.000 | R$ 10.500 |

### Faixa C — Cameras a 300 km+ (so se valor justificar)
| Servidores | Preco anual minimo |
|------------|-------------------|
| Qualquer porte | R$ 15.000+ (negociar caso a caso) |

> **Regra:** escopo completo (7 itens tipo Rodeiro: PGR + PCMSO + LTCAT + LIP + ASOs + eSocial + Palestras) tem piso real de **R$ 10.000–12.000** para cameras dentro de 150 km. Para cameras a 300 km+, piso sobe para **R$ 15.000+**.

**Todos abaixo de R$ 59.906 → cabem em dispensa art. 75 II.**

> ⚠️ Rodeiro (R$ 6.208 para escopo completo a 350 km) = prejuizo de R$ 4.000–5.700. Confirmado NO-GO apos leitura do edital de 54 paginas.
> ⚠️ Conferir com o skill `higilabor-precificacao` antes de rodar em escala — garantir que o piso de custo real (horas Kleber + medica + admin) esta coberto nessas faixas.
...
| **Distancia > 200 km + ASOs presenciais + deslocamento contratada** | **Filtro de viabilidade (secao 2.5) — nao disputar se valor < R$ 15k/ano** |
| **Edital exige equipe minima 3+ (Eng+Med+TST)** | **Higilabor nao tem TST — avaliar custo de terceirizacao antes de disputar** |
| **Subcontratacao vedada em edital distante** | **Elimina opcao de parceiro local — NO-GO automatico se distancia > 200 km** |
...
8. ✅ ~~**Decisão Rodeiro**~~ → **NO-GO confirmado 09/04** — edital de 54 pag. lido integralmente. Prejuizo R$ 4–5,7k. Deal-breakers: distancia 350 km + equipe minima 3 + subcontratacao vedada + ASOs presenciais + R$ 6.208 para escopo completo 7 itens.
9. ⬜ **Enriquecer top 22 P1** da lista 80 câmaras (URLs, presidente, email) antes da primeira onda de prospecção
10. ✅ ~~**Ler edital Rodeiro em PDF**~~ → lido 09/04, todas as pendencias fechadas (ver item 8)

### Status pipeline ativo (09/04/2026)

| Oportunidade | Status | Proxima acao |
|-------------|--------|-------------|
| **Rodeiro/MG** — Dispensa R$ 6.208 | **NO-GO** (edital subprecificado, 350 km, equipe 3, subcont. vedada) | Nenhuma — descartado |
| **Vera Cruz do Oeste/PR** — Dispensa R$ 38.767 | Em analise | Decisao ate 14/04 (encerra 15/04) |
| **IPSETUR Turmalina/MG** — Credenciamento R$ 16.463 | Aguardando ligacao | Ligar para confirmar processo e requisitos |
| **Dom Silverio/MG** — Dispensa R$ 2.006 | **NO-GO** (margem + distancia) | Nenhuma |

### Scan 09/04 — principais achados
- ~~**Rodeiro** (encerra 14/04)~~ → **NO-GO confirmado**
- **Dom Silvério** (encerra 15/04) → ja era NO-GO
...
- **IPSETUR Turmalina/MG** — Credenciamento contínuo até 31/12/2027, R$ 16.463 (entrar por credenciamento é ganho recorrente) → **aguardando ligacao**
- **Vera Cruz do Oeste/PR** — Dispensa PGR+PCMSO+LTCAT+LTIP+AET R$ 38.767, encerra 15/04 (fora do raio MG mas valor bom — avaliar se RT remoto faz sentido) → **em analise**
